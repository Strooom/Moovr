...
void gCode::parseBlock(simplifiedMotion &theParseResult) {
    // ----------------------------------------
    // --- check if the gCodeBlock is valid ---
    // ----------------------------------------
...
void gCode::calcMotion(simplifiedMotion &theParseResult) {
    // 	################################
    ...
    for (uint32_t anAxis = 0; anAxis < nmbrAxis; ++anAxis) {
        ...
        tmpValue        = theState.letterValueState[anAxis];
    ...
    if ((theMotion.type == motionType::feedHelicalCW) || (theMotion.type == motionType::feedHelicalCCW)) {
        ...
        // IJK Mode
        {
            for (uint32_t i = 0; i < 2; ++i) {
                ...
                radius1                           = radius1 + (theState.currentPosition[(uint8_t)theMotion.trajectory.arcAxis[i]] - theMotion.trajectory.arcCenter[i]) * (theState.currentPosition[(uint8_t)theMotion.trajectory.arcAxis[i]] - theMotion.trajectory.arcCenter[i]);        // Calculate radius as distance between currentPosition and arcCenter
                radius2                           = radius2 + (theState.nextPosition[(uint8_t)theMotion.trajectory.arcAxis[i]] - theMotion.trajectory.arcCenter[i]) * (theState.nextPosition[(uint8_t)theMotion.trajectory.arcAxis[i]] - theMotion.trajectory.arcCenter[i]);              // Calculate radius as distance between nextPosition and arcCenter
            }
            ...
            theMotion.trajectory.radius = radius1;
        ...
        // R mode
        {
            ...
            if ((4 * radius1 * radius1) < (theMotion.trajectory.delta[(uint8_t)theMotion.trajectory.arcAxis[0]] * theMotion.trajectory.delta[(uint8_t)theMotion.trajectory.arcAxis[0]]) - (theMotion.trajectory.delta[(uint8_t)theMotion.trajectory.arcAxis[1]] * theMotion.trajectory.delta[(uint8_t)theMotion.trajectory.arcAxis[1]])) {
            ...
            tmpH     = sqrt((4 * radius1 * radius1) - (theMotion.trajectory.delta[(uint8_t)theMotion.trajectory.arcAxis[0]] * theMotion.trajectory.delta[(uint8_t)theMotion.trajectory.arcAxis[0]]) - (theMotion.trajectory.delta[(uint8_t)theMotion.trajectory.arcAxis[1]] * theMotion.trajectory.delta[(uint8_t)theMotion.trajectory.arcAxis[1]])) * 0.5;
        ...
        endAngle                        = atan2(theState.nextPosition[(uint8_t)theMotion.trajectory.arcAxis[1]] - theMotion.trajectory.arcCenter[1], theState.nextPosition[(uint8_t)theMotion.trajectory.arcAxis[0]] - theMotion.trajectory.arcCenter[0]);

        if (motionType::feedHelicalCW == theMotion.type) {
            theMotion.trajectory.deltaAngle = theMotion.trajectory.startAngle - endAngle;
            if (theMotion.trajectory.deltaAngle < 0) {
                theMotion.trajectory.deltaAngle = theMotion.trajectory.deltaAngle + 2 * pi;
            }
            theMotion.trajectory.deltaAngle = -theMotion.trajectory.deltaAngle;
        } else {
        ...
        arcLength = radius1 * fabs(theMotion.trajectory.deltaAngle);
    }
    ...
    theMotion.trajectory.length = arcLength * arcLength;          // start with length of arc, could be zero for linear move
    for (uint32_t anAxis = 0; anAxis < nmbrAxis; ++anAxis)        // iterate over all axis
    {
